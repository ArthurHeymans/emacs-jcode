...
(ert-deftest jcode-host-local-directory-strips-tramp-prefix ()
  (should (equal (jcode--host-local-directory "/ssh:test-host:/tmp/project")
                 "/tmp/project/")))

(ert-deftest jcode-acp-load-params-use-host-local-remote-cwd ()
  (let ((session (jcode--make-session :cwd "/ssh:test-host:/tmp/project")))
    (should (equal (jcode--acp-load-params session "abc")
                   '(:sessionId "abc" :cwd "/tmp/project/")))))
...
(ert-deftest jcode-command-reopens-resumed-project-buffer ()
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'jcode--project-directory) (lambda () root))
            ...
            (jcode))
          (should shown)
          (should (eq (car shown) resumed-chat))
          (should (eq (cdr shown) resumed-input))
          (with-current-buffer (car shown)
            (should (equal jcode--display-session-id "session-resumed"))))
...
(ert-deftest jcode-native-open-session-polls-only-passive-connections ()
  (let ((chat (generate-new-buffer " *jcode-test-poll-chat*"))
        (input (generate-new-buffer " *jcode-test-poll-input*"))
        requests timers process)
    (unwind-protect
        (progn
          (with-current-buffer chat (jcode-chat-mode))
          (with-current-buffer input (jcode-input-mode))
          (cl-letf (((symbol-function 'jcode-native-socket-path) (lambda (&rest _) "/tmp/jcode.sock"))
                    ((symbol-function 'jcode-native--open-process)
                     (lambda (&rest _)
                       (setq process (start-process "jcode-test-cat" nil "cat"))))
                    ((symbol-function 'jcode-native--request)
                     (lambda (_connection type &rest _fields) (push type requests) 1))
                    ((symbol-function 'run-with-timer)
                     (lambda (&rest args) (push args timers) 'fake-timer))
                    ((symbol-function 'cancel-timer) #'ignore)
                    ((symbol-function 'jcode-native-apply-defaults) #'ignore))
            (let ((jcode-native-take-over-active-session t))
              (jcode-native-open-session "s-owned" default-directory chat input)
              (should-not timers))
            (when (process-live-p process) (delete-process process))
            (setq process nil requests nil timers nil)
            (let ((jcode-native-take-over-active-session nil))
              (let ((connection (jcode-native-open-session "s-passive" default-directory chat input)))
                (should timers)
...
(ert-deftest jcode-native-open-session-sends-host-local-remote-working-dir ()
  (let ((chat (generate-new-buffer " *jcode-test-remote-cwd-chat*"))
        (input (generate-new-buffer " *jcode-test-remote-cwd-input*"))
        requests process)
    (unwind-protect
        (progn
          (with-current-buffer chat (jcode-chat-mode))
          (with-current-buffer input (jcode-input-mode))
          (cl-letf (((symbol-function 'jcode-native-socket-path)
                     (lambda (&rest _) "/ssh:test-host:/run/user/1000/jcode.sock"))
                    ((symbol-function 'jcode-native--open-process)
                     (lambda (&rest _)
                       (setq process (start-process "jcode-test-cat" nil "cat"))))
                    ((symbol-function 'jcode-native--request)
                     (lambda (_connection type &rest fields)
                       (push (cons type fields) requests)
                       1))
                    ((symbol-function 'run-with-timer)
                     (lambda (&rest _args) 'fake-timer))
                    ((symbol-function 'cancel-timer) #'ignore)
                    ((symbol-function 'jcode-native-apply-defaults) #'ignore))
            (let ((connection (jcode-native-open-session
                               nil "/ssh:test-host:/tmp/project" chat input)))
              (jcode-native-close connection)
              (let ((subscribe (assoc "subscribe" requests)))
                (should subscribe)
                (should (equal (plist-get (cdr subscribe) :working_dir)
                               "/tmp/project/"))))))
      (when (and process (process-live-p process)) (delete-process process))
      (kill-buffer chat)
      (kill-buffer input))))
...
(ert-deftest jcode-list-aggregate-keeps-duplicate-session-ids-distinct ()
  (let ((process (start-process "jcode-test-live-remote" nil "sleep" "30")))
    (unwind-protect
        (progn
          (cl-progv '(tramp-methods) '(nil)
            (cl-letf (((symbol-function 'file-remote-p)
                       (lambda (file &optional identification _connected)
                         (when (and (stringp file) (string-prefix-p "/rpc:test:" file))
                      ...
                      ((symbol-function 'tramp-dissect-file-name)
                       (lambda (_file &optional _nodefault) 'test-vec))
...
(ert-deftest jcode-list-source-directories-includes-explicit-extra-sources ()
  (let ((jcode-list-extra-source-directories '("/rpc:remote-test:~/")))
    (cl-progv '(tramp-methods) '(nil)
      (cl-letf (((symbol-function 'file-remote-p)
                 (lambda (file &optional identification _connected)
                   (when (and (stringp file) (string-prefix-p "/rpc:" file))
                     (let ((host (cadr (split-string file ":"))))
                       (pcase identification
                         ('method "rpc")
                         ('user nil)
                         ('host host)
...
(ert-deftest jcode-list-aggregate-skips-explicit-stale-remote-without-querying ()
  (let ((jcode-list-extra-source-directories '("/rpc:stale:~/"))
        queried)
    (cl-progv '(tramp-methods) '(nil)
      (cl-letf (((symbol-function 'file-remote-p)
                ...
                ((symbol-function 'jcode-list-sessions-data)
                 (lambda (directory)
                   ...
                   (list (jcode--make-session-info :id directory :updated-at directory)))))
        (should (member "/rpc:stale:~/" (jcode-list-source-directories "~/")))
        (let ((sessions (jcode-list-sessions-data-aggregate
                         (jcode-list-source-directories "~/"))))
          (should (equal queried '("~/")))
          (should (= (length sessions) 1))
          (should (equal (jcode-session-info-id (car sessions)) "~/")))))))

(ert-deftest jcode-list-aggregate-queries-live-remote ()
  (let ((process (start-process "jcode-test-live-remote" nil "sleep" "30"))
        queried)
    (unwind-protect
        (progn
          (cl-progv '(tramp-methods) '(nil)
            (cl-letf (((symbol-function 'file-remote-p)
                       (lambda (file &optional identification _connected)
                         (when (and (stringp file) (string-prefix-p "/rpc:live:" file))
                           (pcase identification
                             ('method "rpc")
                             ('user nil)
                             ('host "live")
                             ...
                             (_ "/rpc:live:")))))
                      ((symbol-function 'tramp-dissect-file-name)
                       (lambda (_file &optional _nodefault) 'live-vec))
                      ((symbol-function 'tramp-get-connection-process)
                       (lambda (_vec) process))
                      ((symbol-function 'jcode-list-sessions-data)
                       (lambda (directory)
                         (push directory queried)
                         (list (jcode--make-session-info :id directory :updated-at directory)))))
              (let ((sessions (jcode-list-sessions-data-aggregate '("~/" "/rpc:live:~/"))))
                (should (equal (sort queried #'string<) '("/rpc:live:~/" "~/")))
                (should (= (length sessions) 2))))))
      (when (process-live-p process) (kill-process process)))))

(ert-deftest jcode-list-source-directories-canonicalizes-localhost-remotes ()
  (cl-letf (((symbol-function 'file-remote-p)
             (lambda (file &optional identification _connected)
               (when (and (stringp file) (string-prefix-p "/rpc:localhost:" file))
                 (pcase identification
                   ('method "rpc")
                   ('user nil)
                   ('host "localhost")
                   ...
                   ('hop nil)
                   (_ "/rpc:localhost:"))))))
    (should (equal (jcode--normalize-list-source-directory "/rpc:localhost:~/") "~/"))))
  ...
  (let ((jcode-config-file (make-temp-file "jcode-config" nil ".toml" "[provider]\n"))
    ...
    (unwind-protect
        (progn
          ...
          (should (equal (jcode-native-config-provider-value "openai_reasoning_effort") "high"))
...
(ert-deftest jcode-native-config-provider-values-roundtrip ()
  (let ((jcode-config-file (make-temp-file "jcode-config" nil ".toml"
                                           "[provider]\nopenai_reasoning_effort = \"low\"\n")))
    (unwind-protect
        (progn
          (should (equal (jcode-native-config-provider-value "openai_reasoning_effort") "low"))
          (jcode-native-config-set-provider-value "default_provider" "openai-oauth")
          (jcode-native-config-set-provider-value "default_model" "openai-oauth:gpt-5.5")
          (should (equal (jcode-native-config-provider-value "default_provider") "openai-oauth"))
          (should (equal (jcode-native-config-provider-value "default_model") "openai-oauth:gpt-5.5"))
          (jcode-native-config-set-provider-value "default_provider" nil)
  ...
  (let ((jcode-config-file (make-temp-file "jcode-config" nil ".toml" "[provider]\n")))
    (unwind-protect
        (cl-letf (((symbol-function 'message) #'ignore))
          (jcode-set-default-provider "openai-oauth")
          (jcode-set-default-model "openai-oauth:gpt-5.5")
          (jcode-set-default-reasoning-effort "low")
          (should (equal (jcode-native-config-provider-value "default_provider") "openai-oauth"))
          (should (equal (jcode-native-config-provider-value "default_model") "openai-oauth:gpt-5.5"))
          (should (equal (jcode-native-config-provider-value "openai_reasoning_effort") "low"))
          (should (equal (jcode--menu-desc-default-provider) "default provider: openai-oauth"))
          (should (equal (jcode--menu-desc-default-effort) "default effort: low")))
      (delete-file jcode-config-file))))

(ert-deftest jcode-native-configured-effort-seeds-header-default ()
  (let ((jcode-config-file (make-temp-file "jcode-config" nil ".toml"
                                           "[provider]\nopenai_reasoning_effort = \"low\"\n")))
    (unwind-protect
        (should (equal (jcode-native--configured-reasoning-effort) "low"))
      (delete-file jcode-config-file))))
